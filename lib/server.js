...
const startServer = (port) => {
  net.createServer(socket => {
    ...
    console.log('SERVER listening @ ', port || 2006)
  })
}
...
const processMiddleware = (MIDDLEWARES, request, response) => {
  console.log('<processMiddleware> Entry MIDDLEWARES = ', MIDDLEWARES.toString())
  if (MIDDLEWARES.length > 0) {
    MIDDLEWARES[0](request, response, (error) => {
      console.log('!!!!!!!!!!!!!!!!!!! <processMiddleware> error = ', error)
      if (!error) {
        processMiddleware(MIDDLEWARES.slice(1), request, response)
        console.log('<processMiddleware> After recursive call..................')
      } else if (error === 'EOF') {
...
const getHandler = (request, response) => {
  ...
  if (typeof ROUTES.GET[request.header.URI] === 'function') {
    ...
    console.log('<getHandler> In else before error404Handler.......')
    error404Handler(request, response)
  }
}
