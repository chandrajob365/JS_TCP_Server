...
const dataEventHandler = (socket, request) => {
  ...
  socket.on('data', rawRequest => {
    ...
    data += rawRequest
    // let headerChunk = rawRequest.slice(0, rawRequest.indexOf('\r\n\r\n')).toString()
    //
    //
    // console.log('<dataEventHandler> headerChunk = ', headerChunk)
    // console.log('<dataEventHandler> headerChunk.indexOf(\'Content-Length\')=', headerChunk.indexOf('Content-Length'))
    // if(headerChunk.indexOf('Content-Length') !== -1) {
    //   var contentLength = headerChunk.match(/Content-Length: \d+/)[0].split(':')[1].trim()
    //   let bodyChunkLength = getBodyChunk(rawRequest).length
    //   console.log('<dataEventHandler> contentLength=', contentLength)
    //   console.log('<dataEventHandler> bodyChunkLength=', bodyChunkLength)
    //   while (bodyChunkLength < contentLength) {
    //     bodyChunkLength = getBodyChunk(rawRequest).length
    //     console.log('<dataEventHandler> Inside while loop bodyChunkLength = ', bodyChunkLength)
    //   }
    //   requestHandler(request, rawRequest)
    // } else {
    clearTimeout(timer)
    timer = setTimeout(() => requestHandler(socket, data), 1)
      //requestHandler(request, rawRequest)
    //}
...
const getHeaderChunk = (rawRequest) => {
  ...
  //let headerChunk = rawRequest.slice(0, newLineIndex)
  let headerChunk = new Buffer(newLineIndex)
  for (let i = 0; i < newLineIndex; i++) {
    headerChunk[i] = rawRequest[i]
  }
  console.log('<getHeaderChunk> headerChunk=', headerChunk.toString())
  return headerChunk.toString()
}

const getBodyChunk = rawRequest => {
  console.log('<getBodyChunk> Entry rawRequest = ', rawRequest.toString())
  let newLineIndex = rawRequest.indexOf('\r\n\r\n')
  console.log('<getBodyChunk> newLineIndex=', newLineIndex)
  // let bodyChunk = rawRequest.slice(newLineIndex + 4, rawRequest.length)
  //
  ...
  console.log('<getBodyChunk> stringified bodyChunk = ', bodyChunk.toString())
  return bodyChunk
}
const parseRequestLine = (request, requestLine) => {
  requestLine = requestLine.split(' ')
  console.log('<parseRequestLine> requestLine=', requestLine)
...
const parseRequestHeader = (request, requestHeaders) => {
  ...
  console.log('<parseRequestHeader> request = ', request)
}

/* msgBody is a string */
const parseRequestBody = (request, bodyChunk) => {
  if ((request.header['Content-Type'] === 'application/x-www-form-urlencoded')) {
    console.log('<parseRequestBody> Content-Type = application/x-www-form-urlencoded')
...
const multipartFormParser = (request, bodyChunk) => {
  console.log('<multipartFormParser> Entry')
  ...
  let multipartChunk = bodyChunk.slice((boundaryIndex + '--'.length + request.header['boundary'].length),
                                bodyChunk.length)
  console.log('<multipartChunk> multipartChunk.toString().indexOf(\'--\') =', multipartChunk.toString().indexOf('--'))
...
const partParser = (request, multipartChunk) => {
  ...
  if (partChunk.indexOf('Content-Type') !== -1) {
    console.log('<partParser> Content-Type exists')
    multipartWithContentType(request, partChunk)
  } else {
    console.log('<partParser> Content-Type doesn\'t exist')
...
/* Write uploaded file to 'upload' location on server */
  fs.writeFile(file['location'], formPartData, function (err) {
    console.log('<multipartWithContentType> Callback from writing to file')
...
const createSession = (request, response, cb) => {
  let cookie = uuidv4()
  console.log('<createSession> cookie=', cookie)
  response['Set-Cookie'] = cookie
  SESSION[cookie] = {}
  console.log('<createSession> After adding SESSION=', SESSION)
  cb(SESSION, cookie)
}
...
const addSession = (request, content) => {
  ...
  console.log('<addSession> SESSION = ', SESSION)
}
...
const sessionHasCookie = browserCookies => {
  ...
  for (let key in SESSION) {
    console.log('<sessionHasCookie> key =', key, ' browserCookies=', browserCookies)
