...
const requestHandler = (request, rawRequest) => {
  ...
  if (request.header.hasOwnProperty('Origin')) corsHandler(request, response)
...
const optionHandler = (request, response) => {
  if (request.header.hasOwnProperty('Origin')) {
    if (ALLOWED_ORIGIN_WITH_METHODS.hasOwnProperty(request.header.Origin)) {
      ...
      if (request.header['Access-Control-Allow-Headers']) {
        response['Access-Control-Allow-Headers'] = request.header['Access-Control-Allow-Headers']
      }
    }
  }
