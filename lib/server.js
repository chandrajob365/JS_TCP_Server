...
const parseRawRequest = (request, requestString) => {
  console.log('<parseRawRequest> requestString = ', requestString)
  ...
  if (request.header.URI.indexOf('?') !== -1) {
    ...
    let queryParams = (request.header.URI)
                    .slice((request.header.URI.indexOf('?') + 1)
                            , request.header.URI.length)
    request.header.URI = baseRoute
    queryParamParser(request, response, queryParams)
  }
...
const queryParamParser = (request, response, queryParams) => {
  queryParams = queryParams.split('&')
  queryParams.forEach(elm => {
    let param = elm.split('=')
